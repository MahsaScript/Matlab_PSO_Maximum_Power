...
for ii=1:length(New_IR_Change)
    set_param('PSOBasedMPPTModel/Constant','Value',num2str(New_IR_Change(ii)));     
    par=20:10:60;
    for jj=1:length(par)
        set_param('PSOBasedMPPTModel/Pulse Generator','PulseWidth',num2str(par(jj)));
        sim('PSOBasedMPPTModel.slx');
